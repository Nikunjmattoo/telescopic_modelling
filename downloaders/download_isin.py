...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('download_isin.log'),
...
def get_isin_data(ticker_symbol, max_retries=3, initial_delay=1):
    """Fetch ISIN data for a given ticker with retry logic."""
    try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if isin:
                    ...
                    logger.warning(f"No ISIN data available for {ticker_clean}")
                    return {
                        'ticker': ticker_clean,
                        'isin': None,
                        'timestamp': datetime.now().isoformat(),
                ...
                if "404" in str(e):
                    logger.warning(f"ISIN data not found for {ticker_clean} (404)")
                    return {
                        'ticker': ticker_clean,
                        'isin': None,
                        'timestamp': datetime.now().isoformat(),
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(f"Failed to fetch ISIN for {ticker_clean} after {max_retries} attempts")
...
def main(force_download=False):
    try:
        ...
        # Process each ticker
        with tqdm(total=len(tickers), desc="Downloading ISIN data") as pbar:
            for i, ticker in enumerate(tickers, 1):
                ...
                try:
                    # Get ISIN data
                    data = get_isin_data(ticker_clean)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Download ISIN data for NSE stocks.')
    ...
    logger.info("Starting ISIN data download...")
    if args.force:
